---
title: Installation
description: Getting started with PortUI
icon: ListStart
---

import { Tab, Tabs } from "fumadocs-ui/components/tabs";

### 1. Install utilities

All components use [Tailwind CSS v4](https://tailwindcss.com/docs/installation/framework-guides/nextjs), so ensure it's installed in your project.
Many components also use the `cn` utility function—install it with the following command:

<Tabs items={["bun", "npx", "pnpm"]}>
  <Tab value="bun">
    ```bash bunx shadcn@latest add https://portui.vercel.app/r/utils.json ```
  </Tab>
  <Tab value="npx">
    ```bash npx shadcn@latest add https://portui.vercel.app/r/utils.json ```
  </Tab>
  <Tab value="pnpm">
    ```bash pnpm dlx shadcn@latest add https://portui.vercel.app/r/utils.json
    ```
  </Tab>
</Tabs>

### 2. That's it 🎉

We use [lucide-icons](https://lucide.dev/guide/installation) for most components that include icons, along with some [shadcn/ui](https://ui.shadcn.com/) components. These dependencies will be automatically installed when using the CLI.

For example, to add `card-05` to your project, it will be as easy as:

<Tabs items={["bun", "npx", "pnpm"]}>
  <Tab value="bun">
    ```bash bunx shadcn@latest add https://portui.vercel.app/r/card-05.json ```
  </Tab>
  <Tab value="npx">
    ```bash npx shadcn@latest add https://portui.vercel.app/r/card-05.json ```
  </Tab>
  <Tab value="pnpm">
    ```bash pnpm dlx shadcn@latest add https://portui.vercel.app/r/card-05.json
    ```
  </Tab>
</Tabs>

_While we provide a convenient 'copy' button for the code, we strongly recommend using the CLI for installing components, as it ensures all necessary files are included._

Add to your page and it works!

```jsx
import Card05 from "@/components/portui/card-05";

export default function Page() {
  return <Card05 />;
}
```

### 3. Blocks

Blocks are a set of components to make a complete block; they contain multiple files. To install them, it is mandatory to use the `CLI`. For example, if you'd like to install the `ai-card-generation` block:

<Tabs items={["bun", "npx", "pnpm"]}>
  <Tab value="bun">
    ```bash bunx shadcn@latest add
    https://portui.vercel.app/r/ai-card-generation.json ```
  </Tab>
  <Tab value="npx">
    ```bash npx shadcn@latest add
    https://portui.vercel.app/r/ai-card-generation.json ```
  </Tab>
  <Tab value="pnpm">
    ```bash pnpm dlx shadcn@latest add
    https://portui.vercel.app/r/ai-card-generation.json ```
  </Tab>
</Tabs>

Then, add to your page and it works!

```jsx
/**
 * AICardGeneration requires 5 additional components
 * that will be automatically installed with the CLI.
 */
import AICardGeneration from "@/components/portui/AICardGeneration";

export default function Page() {
  return <AICardGeneration />;
}
```

### 4. Optional Dependencies

Some components require additional libraries, listed at the bottom of each component. Make sure to install them to ensure the component works properly.

### 5. Monorepo

For monorepos, the `shadcn/ui` CLI contains the `-c` option to specify the path to your workspace. For example:

<Tabs items={["bun", "npx", "pnpm"]}>
  <Tab value="bun">
    ```bash bunx shadcn@latest add https://portui.vercel.app/r/card-05.json -c
    ./apps/www ```
  </Tab>
  <Tab value="npx">
    ```bash npx shadcn@latest add https://portui.vercel.app/r/card-05.json -c
    ./apps/www ```
  </Tab>
  <Tab value="pnpm">
    ```bash pnpm dlx shadcn@latest add https://portui.vercel.app/r/card-05.json
    -c ./apps/www ```
  </Tab>
</Tabs>
